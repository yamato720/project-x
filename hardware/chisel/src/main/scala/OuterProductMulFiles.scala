...
object OuterProductMulFiles {
  ...
  private val header: String =
    """#ifndef PROJECTX_OUTER_PRODUCT_MUL_HPP
      ...
      |
      |// Generated by OuterProductMulFiles.scala.
      |//
      |// 这是历史上的“逐元素外积” black-box 声明。
      |// 当前 Project-X 的 kernel 已经切到 outer_product_tile_bits(...)，
      |// 所以这份头文件没有活跃调用点；保留它主要是为了：
      |// 1. 作为最小单元素 Chisel/HLS black-box 示例
      |// 2. 在需要做逐元素对比实验时还能直接复用
      |//
      |// 参数含义：
      |// - lhs_bits: 一个 lhs 标量，按 IEEE-754 double bit pattern 编码
      |// - rhs_bits: 一个 rhs 标量，按 IEEE-754 double bit pattern 编码
      |// - result_bits: 乘法结果，也按 IEEE-754 double bit pattern 编码
      |//
  ...
  private val model: String =
    """#include "outer_product_mul.hpp"
      ...
      |// 当前没有活跃 kernel 调它，但只要 header 被引用，就仍然需要一个同名定义。
